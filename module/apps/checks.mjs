...
export class PENCheck {
  ...
  static async normaliseRequest(options) {
    ...
    let config = {
      ...
      dialogTemplate: "systems/Pendragon/templates/dialog/rollOptions.hbs",
      chatTemplate: "systems/Pendragon/templates/chat/roll-result.hbs",
    ...
    if (config.shiftKey) {
      ...
      if (usage) {
        ...
        if (config.cardType === CardType.FIXED) {
          config.fixedOpp = Number(usage.fixedOpp);
        }
        config.inquiry = usage.inquiry;
        let tempDecision = usage.decisionChoice;
