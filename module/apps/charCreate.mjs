...
export class PENCharCreate {
  ...
  static async startCreate (actor,step,undo){
    ...
    if (undo) {
      await PENCharCreate.undoCreate(actor, step)
      return
    }
    ...
    if (step === 'addName') {
      let result = await PENCharCreate.addName(actor)
      if (!result) {return}
      ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.create.addName'))
      return      
    }

    //Add Gender -  Get the character gender
    if (step === 'addGender') {
      let result = await PENCharCreate.addGender(actor)
      if (!result) {return}
      ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.create.addGender'))
      return      
    }

    //Add Lord -  Get the character's Leige Lord
    if (step === 'addLord') {
      let result = await PENCharCreate.addLord(actor)
      if (!result) {return}
      ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.create.addLord'))
      return      
    }

    //Add Home/Residence -  Get the character's home (not homeland)
    if (step === 'addHome') {
      let result = await PENCharCreate.addHome(actor)
      if (!result) {return}
      ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.create.addHome'))
      return      
    }    

    //Add Starting Equipment from Class
    if (step === 'addEquip') {
      let result = await PENCharCreate.addEquip(actor)
      if (!result) {return}
      ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.create.addEquip'))
      return      
    }    
    ...
    if (step === "8") {
      //If actor has a homeland then stop
    ...
    //STEP 9:  Set Parent Passions (optional)
    if (step === "9") {
    ...
    //Heir Status
    if (step === "addHeir") {
      let result = await PENCharCreate.addHeir(actor)
      if (!result) {return}
      ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.create.addHeir'))
    }
  ...
  //----------------UNDO STEPS------------------------------
  //Undo Create routine
  static async undoCreate (actor,step){
    //If not GM then return
    if (!game.user.isGM){return}
    let confirmation = await PENUtilities.confirmation(game.i18n.localize("PEN.step." + step) + " " + game.i18n.localize('PEN.undoStep'))
    ...
    switch (step) {
      ...
      //Reset character Gender
      case "addGender":
        await PENCharCreate.undoGender(actor)
        ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.undo.addGender'))
        break;   
        

      //Reset character Liege Lord
      case "addLord":
        await PENCharCreate.undoLord(actor)
        ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.undo.addLord'))
        break;   
        
      //Reset character Home
      case "addHome":
        await PENCharCreate.undoHome(actor)
        ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.undo.addHome'))
        break;           

      //Reset Heir Status
      case "addHeir":
        await PENCharCreate.undoHeir(actor)
        ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.undo.addHeir'))
        break;   

      //Reset Starting Equipment
      case "addEquip":
        await PENCharCreate.undoEquip(actor)
        ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.undo.addEquip'))
        break;   
      ...
      case "1":
        ...
        await PENCharCreate.undoClass(actor, false)
      ...
      //Reset Knightly Class
      case "addClass":
        await PENCharCreate.undoClass(actor, false)
        ui.notifications.warn(actor.name + ": " + game.i18n.localize('PEN.undo.addClass'))
        break
    }    
    return
  }
  ...
  //Add a Character Name Method------------------------------------------------
  static async addName(actor) {
    let newName = await PENCharCreate.inpValue(game.i18n.localize('PEN.charName'));
    ...
    //Update PID icon
    const html = $(actor.sheet.element).find('header.window-header .edit-pid-warning,header.window-header .edit-pid-exisiting')
      if (html.length) {
        html.css({
          color: (key ? 'white' : 'red')
        })
  ...
  //Add Character Gender  Method------------------------------------------------
  static async addGender(actor) {
    let newName = await PENCharCreate.inpValue(game.i18n.localize('PEN.charGender'));
  ...
  //Add Leige Lord  Method------------------------------------------------
  static async addLord(actor) {
    let newName = await PENCharCreate.inpValue(game.i18n.localize('PEN.charLord'));
    if (!newName) return false;
    await actor.update({
  ...
  //Add Character Home/Residence  Method------------------------------------------------
  static async addHome(actor) {
    let newName = await PENCharCreate.inpValue(game.i18n.localize('PEN.charHome'));
    if (!newName) return false;
    await actor.update({
  ...
  static async addEquip(actor){
    ...
    if (actClass.length<1) {return false}
    let newItems = [];
  ...
  static async step1(actor) {
    let data = {
      ...
      button1: {label:game.i18n.localize("PEN.random"),
              icon: "fas fa-dice"},
      button2: {label:game.i18n.localize("PEN.constructed"),
    ...
    await actor.update({'system.create.random': option,
                        'system.create.step' :2})
    return true
  }                      
  ...
  static async step7(actor){
    ...
    let mainList = await game.system.api.pid.fromPIDRegexBest({ pidRegExp: new RegExp('^i.' + PENUtilities.quoteRegExp('class') + '.+$'), type: 'i' })
    let newList = mainList.filter(i=>i.system.starter).map(itm=>{return{name:itm.name, pid: itm.flags.Pendragon.pidFlag.id}})
    ...
    await PENCharCreate.addClass (actor, itemData[0],false, false, true)
    return true
  }
  ...
  static async stepAddClass(actor){
    ...
    let currentClass = actor.system.class
    if (actor.system.classID  === "") {
      await PENCharCreate.addClass (actor, itemData[0],false, false, true) //add new Class, don't ask for passion method, don't add equip, but do add Passions
    } else {
      let currentClass = await actor.items.get(actor.system.classID)
      let newClass = itemData[0]
      if (currentClass) {
        if (itemData[0].flags?.Pendragon?.pidFlag?.id === 'i.class.mercenary-knight') {
          let oldPassion = (await actor.items.filter(i=>['i.passion.homage-lord'].includes(i.flags?.Pendragon?.pidFlag?.id)))[0]
          let newPassion = (await actor.items.filter(i=>['i.passion.fealty-lord'].includes(i.flags?.Pendragon?.pidFlag?.id)))[0]
          //If actor doesn't have Fealty then add passion
          if (!newPassion) {
            let newItems = [];
            let passionList = await game.system.api.pid.fromPIDBest({ pidRegExp: 'i.passion.fealty-lord' });
            newItems.push(passionList[0])
            newPassion = await actor.createEmbeddedDocuments("Item", newItems);
          }
          if(oldPassion) {
            let score = oldPassion.system.value
            await oldPassion.update({'system.value': 0});
            await newPassion.update({'system.value': score});   
          }
        }
        await currentClass.delete();
        await PENCharCreate.addClass (actor, itemData[0],false, false, false) //add new Class, don't ask for passion method,  don't add equip and skip passions        
      }  
    }
    return true
  }
  ...
  static async step9(actor){
    ...
    let bonus = 0
    if (parentPass != false) {
      bonus = Number((await PENCharCreate.inpValue(game.i18n.localize('PEN.parentPassion'))).age)
      if (bonus > 15) {
    ...
    for (let item of actor.items) {
      if (item.type === 'skill') {
        ...
        if (item.system.base.stat != 'none') {
          score = Number(score) + Number(Math.round((actor.system.stats[item.system.base.stat].value + actor.system.stats[item.system.base.stat].culture)  * item.system.base.multi))
        }
  ...
  static async step13(actor) {
    ...
    //Get age
    let dlg = await PENCharCreate.inpValue(game.i18n.localize('PEN.ageInput'))
    if(!dlg){return false}
  ...
  static async step15(actor) {
    ...
    let lord = await PENCharCreate.inpValue(game.i18n.localize('PEN.lordsGlory'))
  ...
  let itemPID = ""
  if (newList.length <1) {return 'xxx'}
  if (newList.length === 1) {
 ...
 //Function to call the Family Create Dialog box
  ...
  //Reset Character Name Method------------------------------------------------
  static async undoName(actor) {
    let newName = game.i18n.localize('TYPES.Actor.character');
    if (!newName) return false;
    let key = await game.system.api.pid.getPrefix(actor) +  PENUtilities.toKebabCase(newName)
  ...
  //Reset Character Gender Method------------------------------------------------
  static async undoGender(actor) {
    await actor.update({'system.gender': ""})
    return true;
  }

  //Reset Character Lord Method------------------------------------------------
  static async undoLord(actor) {
    await actor.update({'system.lord': ""})
    return true;
  }
  
  //Reset Character Home Method------------------------------------------------
  static async undoHome(actor) {
    await actor.update({'system.home': ""})
    return true;
  }  

  //Reset Heir Status Method------------------------------------------------
  ...
  static async undostep2(actor) {
    ...
    let history = await (actor.items.filter(itm =>itm.type==='history')).filter(itm =>['inherited',"squired"].includes(itm.system.source)).map(itm => {return (itm.id)})
  ...
  static async undoClass(actor,knightly) {
    //If Knightly then delete Passions, if !Knightly then leave passions intact
  ...
  static async undostep13(actor) {
    ...
    await actor.update({
      ...
      'system.features': ""})
    let history = await (actor.items.filter(itm =>itm.type==='history')).filter(itm =>['born'].includes(itm.system.source)).map(itm => {return (itm.id)})
    await Item.deleteDocuments(history, {parent: actor});
    await actor.update({
      'system.born' : 487,
  ...
  static async undoEquip(actor) {
    let equip = await actor.items.filter(itm=>itm.type !='class').filter(itm=>itm.system.source==='class').map(itm => {return (itm.id)})
  ...
  static async twoOptions(data) {
    const html = await foundry.applications.handlebars.renderTemplate('systems/Pendragon/templates/dialog/2buttons.hbs',data)
